function estResults = fft2D(radarParams, cfarConfig, rxGrid, txGrid)
%2D-FFT algorithm for range, velocity estimation.
%
% Input parameters:
%
% radarParams: structure containing radar system parameters 
% such as the number of IFFT/FFT points, range and velocity resolutions.
%
% cfarConfig: containing an object implementing the Constant False Alarm
% Rate (CFAR) detection algorithm and index of the cells under test (CUT)
%
...
% Ministry of Education, BUPT.
    ...
    nIFFT = radarParams.nIFFT;
    nFFT  = radarParams.nFFT;

    % Range and Doppler grid
    rngGrid = ((0:nIFFT-1)*radarParams.rRes)';        % [0,nIFFT-1]*rRes
    dopGrid = ((-nFFT/2:nFFT/2-1)*radarParams.vRes)'; % [-nFFT/2,nFFT/2-1]*vRes

    % CFAR
    cfarDetector = cfarConfig.cfarDetector2D;
    CUTIdx       = cfarConfig.CUTIdx;
    ...
    for r = 1:nAnts
        ...
        for i = 1:nDetecions
            ...
            if ~isempty(detections{r})
                ...
                % Assignment
                estResults(i).rngEst{r} = rngIdx.*radarParams.rRes;
                estResults(i).velEst{r} = velIdx.*radarParams.vRes;
