...
% Ministry of Education, BUPT.
    ...
    %% Topology parameters
    nTargets = 1;%cellSimuParams.numTargets; % number of targets
    ...
    if azi <= 180 && azi >= 60  % Determine the sector scope ,Convert azimuth
       azi   = azi-120;
    end
    if azi <= -60 && azi >= -180
       azi   = azi+120;
    ...
    if isa(txArray, 'parameters.baseStation.antenna.upa')  % UPA model
        ...
        spacingX = txArray.dV * lambda;                 % array X-axis element spacing
        spacingY = txArray.dH * lambda;                 % array Y-axis element spacing
    ...
    radarParams.azimuthScanScale         = 120;         % azimuth scan scale, normally set to 120°, meaning [-60°, 60°]
